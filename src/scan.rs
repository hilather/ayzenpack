    ...
    F: FnMut(&ScannedEntry, Option<Vec<u8>>) -> Result<()>,
{
    ...
    let prefix_len = detect_zip_prefix(path, &mut file)?;
    ...
    let reader = BufReader::with_capacity(ZIP_BUF, ZipView::new(file, prefix_len));
...
pub fn zip_prefix_len(path: &Path) -> Result<u64> {
    let mut file = File::open(path).map_err(|source| io_at(source, path))?;
    detect_zip_prefix(path, &mut file)
}
...
/// EOCD-from-EOF extra-data math. Returns 0 for a normal ZIP; `NotZip` if there is no ZIP.
fn detect_zip_prefix(path: &Path, file: &mut (impl Read + Seek)) -> Result<u64> {
    ...
    let (eocd_off, cd_size, cd_off, entries) = find_cd_bounds(path, file, file_len)?;
    let cd_start = eocd_off.checked_sub(cd_size).ok_or_else(|| not_zip(path))?;
    let prefix_len = cd_start.checked_sub(cd_off).ok_or_else(|| not_zip(path))?;
    if prefix_len > MAX_PREFIX {
        return Err(not_zip(path));
    }

    let empty = entries == 0 && cd_size == 0;
    confirm_zip_at(path, file, prefix_len, empty)?;
    Ok(prefix_len)
}
