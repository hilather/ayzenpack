...
3. Stop-the-line, effectiveness, and `capture-lesson` rules from that skill apply. After the loop, run `record-hint-outcome` if there is signal; otherwise say `no effectiveness signal`.
