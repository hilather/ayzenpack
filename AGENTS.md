...
3. **Never** store a second encoding of the same entry. No default `cdata_blob` next to the content blob. `ZipArchive` count ≠ homemade CD count is a parser bug, not a `raw_zip` case. Crate 0.2.2 never writes `raw_zip` of a listed jar. `raw_zip` only if `ZipArchive` never populated `entries[]` (`UnsupportedArchive` spanning / `NotZip`). Count mismatch / range overlap / slice `Err` on a listed jar is skip-exact (index + CAS / `write_jar`), never whole-zip CAS. Equal-offset last-wins with matching homemade count is exact splice, never `raw_zip`. Do not reintroduce dual copies. That is why packs went 200MB → ~3GB: uncompressed CAS + original deflate streams (zstd cannot shrink those).
...
5. Restore rebuilds a valid ZIP from index + blobs (STORE splice / flate2 or zlib-rs codec hit if it happens / otherwise rebuild). Whole-file `source_*` hashes may change. That is acceptable. Do not add a Java subprocess / vendor `Deflater`; do not add `cdata_blob` for misses; do not `raw_zip` a listed jar. In-process zlib-rs raw-deflate hits are the 0.2.4 path.
...
Crate **0.2.8** / format **v2** groups uncompressed blobs in 4 MiB record-aligned zstd frames and **never writes** `cdata_blob` on STORE/DEFLATE (file or dir, any method). Crate **0.2.2** never writes `raw_zip` of a listed jar. `zip_archive_opens` must accept only the outer listing (rust zip may latch onto a STORE nested EOCD). PK-start outer scan (`prefix_len == 0`) refuses a nested-EOCD latch (homemade count ≠ `ZipArchive::len()` and a first-local-shifted view lists the outer count); same-zip last-wins (`dup.txt`) stays listable. Per-entry codecs: a miss rebuilds that slot only. Crate 0.2.0 leftover MixedExact / ExactWithExotic dual copies still read. Do not add new `cdata_blob` puts. Do not “fix” mix size by storing more cdata.

- Stencil: ratarmount-rs columns (`offsetheader`, `data_start`, `nestedindexes`). Offsets are a write recipe after the source file is gone. Keep `blob` / `local_header_offset` / `tail_blob` / `prefix_blob`.
...
- Codecs (no `cdata_blob`): STORE; `deflate-raw:zlib:{1,3,6,9}` via in-process zlib-rs (not a Java subprocess); existing `deflate-raw:flate2:{1,3,6,9}`; `deflate-raw:stored`. A miss must not drop sibling codecs. `deflate-raw:stored` hits BTYPE 00; do not use that fixture as a miss.
- Do not treat `first offsetheader == prefix_len` as new work (0.2.3 already slices that). Leftover-junk CD (N complete records + trailing junk, count matches ZipArchive) is homemade_ok + `tail_blob` (exact when every slot hits). Remaining homemade-`None` never gets `tail_blob`. Arm 1 (headers present, every slot at recorded csize) is stencil seek + synthetic CD (FileAbs iff `prefix_size > 0`; locals-region identity + FileAbs listing, not original-file `source_*`). Arm 2 is concat + synthetic CD. Arm 3 range overlap / ZipArchive count mismatch (`dup.txt`) is `write_jar` ZipWriter STORE (`method_code == 0` / `zip_index`). Leading pad is `leading_pad_blob` on a PK-start hole (do not extend `prefix_len`). Prefix+hole **(A)** `[non-PK prefix][hole][first CD local]` is already `prefix_blob` covering bash+hole (`find_cd_first_local`); not skip-exact; do not split bash vs hole. Prefix+hole **(B)** `prefix_len > 0 && min(zip_rel) != 0` after convert is a dead defensive `Err` at first-local-not-at-0; keep it; do not call (B) absorbed. Equal-offset last-wins with matching homemade count is exact splice (pad of the unreferenced second local kept; unique content 1). Range overlap / `dup.txt` stay skip-exact arm 3.
- `source_*` must match iff `bit_identical_restore` (always-on: baked zlib-rs bitstream + STORE zip-A after `zip_index`). Homemade-`None` arm 1 is locals-region identity + FileAbs listing, not original-file match. A zopfli / unknown-deflate miss still rebuilds that entry only. Do not put corpus lucene/jackson `source_*` in CI until every method-8 slot is a measured hit (`AYZENPACK_CORPUS_DIR`; not always-on until 100% hits). Mix `proven_miss` must allow sibling codecs.
