...
- Do not treat `first offsetheader == prefix_len` as new work (0.2.3 already slices that). Remaining skip-exact: homemade-`None` (never `tail_blob`; `write_jar`) and leading-pad (`leading_pad_blob`, do not extend `prefix_len`; prefix+hole stays skip-exact). Overlap stays skip-exact.
