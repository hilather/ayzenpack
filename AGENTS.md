...
- Do not treat `first offsetheader == prefix_len` as new work (0.2.3 already slices that). Leftover-junk CD (N complete records + trailing junk, count matches ZipArchive) is homemade_ok + `tail_blob` (exact when every slot hits). Remaining homemade-`None` never gets `tail_blob`. Arm 1 (headers present, every slot at recorded csize) is stencil seek + synthetic CD (FileAbs iff `prefix_size > 0`; locals-region identity + FileAbs listing, not original-file `source_*`). Arm 2 is concat + synthetic CD. Arm 3 range overlap / ZipArchive count mismatch / prefix+hole is `write_jar` ZipWriter STORE (`method_code == 0` / `zip_index`). Leading pad is `leading_pad_blob` (do not extend `prefix_len`; prefix+hole stays skip-exact arm 3). Equal-offset last-wins with matching homemade count is exact splice (pad of the unreferenced second local kept).
