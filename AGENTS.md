...
- Do not treat `first offsetheader == prefix_len` as new work (0.2.3 already slices that). Leftover-junk CD (N complete records + trailing junk, count matches ZipArchive) is homemade_ok + `tail_blob` (exact when every slot hits). Remaining skip-exact: homemade-`None` (never `tail_blob`; `write_jar` STOREs `method_code == 0` / `zip_index`) and leading-pad (`leading_pad_blob`, do not extend `prefix_len`; prefix+hole stays skip-exact). Overlap stays skip-exact.
- `source_*` must match when every slot hits (always-on: baked zlib-rs bitstream + STORE zip-A after `zip_index`). A zopfli / unknown-deflate miss still rebuilds that entry only. Do not put corpus lucene/jackson `source_*` in CI until every method-8 slot is a measured hit. Mix `proven_miss` must allow sibling codecs.
