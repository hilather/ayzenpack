...
- Codecs (no `cdata_blob`): STORE; `deflate-raw:zlib:{1,6,9}` via in-process zlib-rs (not a Java subprocess); existing `deflate-raw:flate2:{1,3,6,9}`; `deflate-raw:stored`. A miss must not drop sibling codecs. `deflate-raw:stored` hits BTYPE 00; do not use that fixture as a miss.
