...
- `source_*` must match when every slot hits (always-on: baked zlib-rs bitstream + STORE zip-A after `zip_index`). A zopfli / unknown-deflate miss still rebuilds that entry only. Do not put corpus lucene/jackson `source_*` in CI until every method-8 slot is a measured hit. Rewrite mix `proven_miss` so sibling codecs are allowed.
