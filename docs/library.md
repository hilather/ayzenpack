...
Restore policy (see [DESIGN.md](https://github.com/hilather/ayzenpack/blob/main/DESIGN.md)): Priorities: (1) lean pack (2) complete rehydrate (3) class-level dedup. `source_*` must match iff `bit_identical_restore`. Outer exact is a file seek-walk (no outer `Vec`). Leftover junk after N complete CD records with `N == ZipArchive::len()` is homemade_ok + `tail_blob` (exact when every slot hits). Remaining homemade-`None` never gets `tail_blob`. Arm 1 homemade-`None` with captured headers is stencil seek + synthetic CD. Arm 3 / arm 2-until-concat ZipWriter STOREs `method_code == 0` / `zip_index` over uncompressed payload (`read_entry_content` / `reconstruct_child_zip`); never `resolve_cdata`. Nested STORE is `zip_index` + shared class blobs — never CAS `blake3(inner zip)`. Do not store `cdata_blob`. Do not chase bit-identical hashes on a miss.
