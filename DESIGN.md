...
A healthy zip -A fat whose first local is the prefix already slices on 0.2.3. Remaining skip-exact: homemade CD parse `None` (`tail_blob` only if every slot hits) and leading pad (`leading_pad_blob`, do not extend `prefix_len`). Decide `zip_index` vs opaque before `--jobs` `spawn_file`. Child stencil is tail-bearing; cap child file entries at 65535. Never CAS the whole inner ZIP if the slot is a `zip_index`.
